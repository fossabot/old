...
void encode_jpeg(jpeg_compress_struct cinfo, cairo_surface_t *surface, int quality, bool progressive, int chromaHSampFactor, int chromaVSampFactor) {
  ...
  image_jpeg_set_defaults(&cinfo);
  if (progressive)
    image_jpeg_simple_progression(&cinfo);
  image_jpeg_set_quality(&cinfo, quality, (quality < 25) ? 0 : 1);
  ...
  free(dst);
  image_jpeg_finish_compress(&cinfo);
  image_jpeg_destroy_compress(&cinfo);
}
...
write_to_jpeg_stream(cairo_surface_t *surface, int bufsize, JpegClosure* closure) {
  ...
  jpeg_error_mgr jerr;
  cinfo.err = image_jpeg_std_error(&jerr);
